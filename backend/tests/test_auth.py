...
def test_cadastro_colaborador_nao_expoe_pin(client, admin_headers):
    resp = _cadastrar(client, admin_headers)
...
def test_pin_persistido_apenas_como_hash(client, admin_headers, db_session):
    _cadastrar(client, admin_headers, matricula="222222", pin="4321")
...
def test_login_sucesso_retorna_jwt(client, admin_headers):
    _cadastrar(client, admin_headers, matricula="333333", pin="1111", perfil="medico")
...
def test_login_pin_incorreto_retorna_401(client, admin_headers):
    _cadastrar(client, admin_headers, matricula="444444", pin="1111")
...
def test_login_colaborador_desligado_retorna_401(client, admin_headers, db_session):
    _cadastrar(client, admin_headers, matricula="888888", pin="1234")
...
def test_cadastro_matricula_duplicada_retorna_409(client, admin_headers):
    _cadastrar(client, admin_headers, matricula="555555", pin="1234")
    resp = _cadastrar(client, admin_headers, matricula="555555", pin="5678")
    assert resp.status_code == 409


def test_cadastro_race_condicao_retorna_409(client, admin_headers, monkeypatch):
    def commit_com_violacao_unique(self):
        raise IntegrityError("INSERT", {}, Exception("unique constraint failed"))

    monkeypatch.setattr(Session, "commit", commit_com_violacao_unique)

    resp = _cadastrar(client, admin_headers, matricula="666666", pin="1234")
    assert resp.status_code == 409


def test_cadastro_matricula_com_formato_invalido_retorna_422(client, admin_headers):
    resp = client.post(
        "/api/colaboradores",
...
def test_cadastro_pin_com_formato_invalido_retorna_422(client, admin_headers):
    resp = client.post(
        "/api/colaboradores",
...
def test_cadastro_perfil_invalido_retorna_422(client, admin_headers):
    resp = client.post(
        "/api/colaboradores",
